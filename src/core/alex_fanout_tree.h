    ...
    namespace fanout_tree {
        ...
        template <class T, class P, class Compare = std::less<T>>
            std::pair<int, double> find_best_fanout_bottom_up(
                ...
                int best_level = 0;
                double best_cost = node->cost_ + kNodeLookupsWeight; // why + kNodeLookupsWeight here?
        ...
        template <class T, class P>
            int find_best_fanout_existing_node(const AlexModelNode<T, P>* parent,
                    ...
                    std::vector<FTNode>& used_fanout_tree_nodes,
                    int max_fanout, ExpeData* c=nullptr) {
                // Repeatedly add levels to the fanout tree until the overall cost of each
                // level starts to increase
                ...
                //Rui: 
                // The high level idea here is that assuming two points[(0, left_boundary_value), (1, right_boundary_value)] to get
                // a base model a(slope) and b(intecept). 
                // a = (y_2 - y_1) / (right_boundary_value - left_boundary_value)
                // b = y_1 - a*x   --->  b  = 0 - 1 * a * left_boundary_value
                LinearModel<T> base_model;
                base_model.a_ = 1.0 / (right_boundary_value - left_boundary_value); //Rui: Assuming y_2 - y_1 = 1? 
                base_model.b_ = -1.0 * base_model.a_ * left_boundary_value;  //Rui: assuming that (left_boundary_value, 0) point to calculate b?
