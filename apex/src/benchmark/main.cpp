...
void Run(){
    ...
    if(keys_file_path.std::string::find("longlat") != std::string::npos) {
           dataset_type = 1;
    } else if(keys_file_path.std::string::find("ycsb") != std::string::npos){
        dataset_type = 2;

    } else if(keys_file_path.std::string::find("lognormal") != std::string::npos) {
        dataset_type = 3;
    }
	...
	char *workload =  generate_workload(operation, generate_num, keys, num_pre_insertion, insert_frac, dataset_type,
    batch_size, poisoning_frac, batch_size, lookup_distribution, theta);


    std::cout<<" check="<< dataset_type<<"\n";
	...
	if(operation == "mixed"){
		...
		range_array[thread_num - 1].num = generate_num - partition_num * (thread_num - 1);
		range_array[thread_num - 1].workload = reinterpret_cast<char*>(my_workload + (thread_num - 1) * partition_num);
                std::cout<<"range_array[thread_num - 1].id="<<range_array[thread_num - 1].id
			 <<", range_array[thread_num - 1].num="<<range_array[thread_num - 1].num<<"\n";
...
int main(int argc, char* argv[]) {
    ...
    batch_size = stoi(get_with_default(flags, "batch_size", "600"));
	...
	index_type = get_required(flags, "index");
        poisoning_frac = stod(get_required(flags, "poisoning_frac"));
